...
export default function ForgotPasswordScreen() {
  ...
  const requestOTP = async () => {
    ...
    try {
      ...
      setSuccess(`SMS sent to ${mobile}. Note the OTP.`);
      setStep(2);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.root} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        {error   ? <View style={styles.errorBox}><Ionicons name="alert-circle" size={16} color={Colors.errorText} /><Text style={styles.errorText}>{error}</Text></View>   : null}
        ...
        {step === 1 && (
          <>
            ...
            <View style={styles.infoBox}>
              <Ionicons name="chatbox-ellipses-outline" size={16} color={Colors.blue600} style={{ marginRight: 10, marginTop: 1 }} />
            ...
            <TouchableOpacity style={styles.submitBtn} onPress={requestOTP} disabled={loading}>
              {loading ? <ActivityIndicator color={Colors.white} /> : <Text style={styles.submitBtnText}>Send SMS with OTP →</Text>}
            </TouchableOpacity>
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  errorText:  { flex: 1, fontSize: 14, color: Colors.errorText },
